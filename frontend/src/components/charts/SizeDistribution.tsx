...
export default function SizeDistribution() {
  ...
  if (!result || sizeBuckets.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-center h-80">
        <span className="text-gray-400">Analyzing key size distribution...</span>
      </div>
    );
  }

  const labels = sizeBuckets.map((b) => b.label);
  const values = sizeBuckets.map((b) => b.count);
